...
public class RenameCommandHandler : CommandHandler
{
    public RenameCommandHandler(FileSystemNavigator navigator, IEnumerable<IObserver> observers)
        : base(navigator, observers)
    {
    }

    public override RenameCommandHandler Handle(ref string[] args)
    {
        ...
        if (args[0].StartsWith("file", StringComparison.CurrentCulture) &&
            args[1].StartsWith("rename", StringComparison.CurrentCulture))
        {
