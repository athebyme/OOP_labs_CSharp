...
public class NeutrinoNebula : Nebula
{
    public NeutrinoNebula(int range, int colonySize, int countAsteroid, int countMeteorite, bool hasSpecialEvent)
        : base(range)
    {
        ...
        SubspaceChannels = new ReadOnlyCollection<SubspaceChannel>(new[] { new SubspaceChannel(Range, true, hasSpecialEvent, null) });
        Colony = new Colony(new SpaceWhales(), colonySize);
